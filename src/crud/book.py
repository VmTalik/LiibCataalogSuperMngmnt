...
class BookCRUDRepository(BaseCRUDRepository):
    ...
    async def get_books_list(self, offset: int = 0, limit: int = 10):
        stmt = select(Book).options(selectinload(Book.author)).offset(offset).limit(limit)
    ...
    async def get_book_by_id(self, book_id: int) -> Book | None:
        stmt = select(Book).where(Book.id == book_id).options(selectinload(Book.author, Book.borrows))
